...
const getSpecsFromDocsAndExamples = async () => {
  ...
  for await (const file of chakra_docs_files) {
    ...
    function getDescriptionByTitle(markdown, title) {
      // Create a regular expression to match the title and capture the following text
      const regex = new RegExp(`^# ${title}\\s*([\\s\\S]*?)(?=^# |$)`, 'im');

      // Execute the regular expression on the markdown text
      const match = regex.exec(markdown);

      if (match && match[1]) {
        return match[1].trim(); // Extracted description
      } else {
        return null; // Title not found or no description
      }
    }

    meta.description = getDescriptionByTitle(content, meta.title);
